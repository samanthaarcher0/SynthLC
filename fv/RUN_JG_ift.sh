...
echo "
if [ -f \"$SVA\" ] && [ -f \"$TOPFILE\" ]; then
    { head -n -1 \"$TOPFILE\"; cat \"$FVMACRO\"; cat \"$SVA\" ; echo \"\" ; tail -n 1 \"$TOPFILE\"; } > \"${TOPV}\" #\"${DIR}/${DUMPNAME}_top.sv\"
else 
...
echo "
if [ -f \"$SVA\" ] && [ -f \"$TOPFILE\" ]; then
    { head -n -1 \"$TOPFILE\"; cat \"$FVMACRO\"; cat \"$HEADER\" ; cat \"$SVA\" ; echo \"\" ; tail -n 1 \"$TOPFILE\"; } > \"${TOPV}\" #\"${DIR}/${DUMPNAME}_top.sv\"
else 
    echo \"[RUN_JG] no property at $SVA is found or no $TOPFILE\"
    exit 0
fi
